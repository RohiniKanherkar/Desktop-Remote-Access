...
def type_box():
    tp_fr = tk.Tk()
    tp_fr.title('Welcome To MITAOE')
    bx_txt = tk.Entry(tp_fr, width=100)
    bx_txt.pack()
    send_but =tk.Button(tp_fr, text="Send The Message", command=lambda:conn.send(('cde:'+bx_txt.get()).encode()))
...
k.destroy()
root = tk.Tk()
root.title('MITAOE Remote Trackpad | CN Mini Project Underguidance of Prof. S M Bhagat Sir')
